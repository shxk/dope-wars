	...
	<v-container>
		...
		<div class="text-center">
			<v-dialog v-model="dialog" width="500">
				<v-card>
					<v-card-title class="headline grey darken-2" primary-title>Game Over</v-card-title>

					<v-card-text style="text-align:center;">You finished the game with:</v-card-text>
					<v-card-text style="text-align:center;">Cash: £{{Number(cash).toLocaleString()}}</v-card-text>
					<v-card-text style="text-align:center;">Bank: £{{Number(bank).toLocaleString()}}</v-card-text>
					<v-card-text style="text-align:center;">Debt: £{{Number(debt).toLocaleString()}}</v-card-text>
					<v-card-text
						style="text-align:center;"
					>Final Amount: £{{Number((cash+bank)-debt).toLocaleString()}}</v-card-text>

					<v-divider></v-divider>

					<v-card-actions>
						<div class="flex-grow-1"></div>
						<v-btn color="primary" text @click="newGame()">New Game</v-btn>
					</v-card-actions>
				</v-card>
			</v-dialog>
		</div>
		...
		<div class="text-center">
			<v-dialog v-model="locationDialog" width="500">
		...
		<div v-if="!policeDialog" class="text-center">
			<v-dialog v-model="eventDialog" width="500">
				<v-card>
					<v-card-title class="headline grey darken-2" primary-title>News bulletin</v-card-title>
					...
					<v-card-actions>
						<div class="flex-grow-1"></div>
						<v-btn color="primary" text @click="eventDialog = false">Okay</v-btn>
					</v-card-actions>
				</v-card>
			</v-dialog>
		</div>
		...
		<!-- Pay debt dialog box  -->
		<div class="text-center">
			<v-dialog v-model="debtDialog" width="500">
				<v-card>
					<v-card-title class="headline grey darken-2" primary-title>Pay Debt</v-card-title>
					...
					<v-card-actions>
						<div class="flex-grow-1"></div>
						<v-btn color="primary" text @click="debtDialog = false">Close</v-btn>
					</v-card-actions>
				</v-card>
			</v-dialog>
		</div>
		...
		<!-- Bank dialog box  -->
		<div class="text-center">
			<v-dialog v-model="bankDialog" width="500">
				<v-card>
					<v-card-title
						class="headline grey darken-2"
						primary-title
					>Bank £{{Number(bank).toLocaleString()}}</v-card-title>

					<v-card-text style="text-align:center;">
						<v-layout v-if="!withdrawSelected && !depositSelected">
							<v-flex mt-3 xs6>
								<v-btn color="warning" :disabled="!bank > 0" @click="withdrawSelected = true">Withdraw</v-btn>
							</v-flex>
							<v-flex mt-3 xs6>
								<v-btn color="success" :disabled="!cash > 0" @click="depositSelected = true">Deposit</v-btn>
							</v-flex>
						</v-layout>

						<v-row v-if="depositSelected">
							<v-col class="pr-4">
								<v-slider v-model="depositAmount" class="align-center" :max="cash" :min="0" hide-details>
									<template v-slot:append>
										<v-text-field
						...
						<v-row v-if="withdrawSelected">
							<v-col class="pr-4">
								<v-slider v-model="withdrawAmount" class="align-center" :max="bank" :min="0" hide-details>
									<template v-slot:append>
										<v-text-field
						...
						<v-flex v-if="depositSelected || withdrawSelected">
							<v-btn color="success" @click="confirmBankTransaction()">
					...
					<v-card-actions>
						<div class="flex-grow-1"></div>
						<v-btn color="primary" text @click="bankDialogReset()">Close</v-btn>
					</v-card-actions>
				</v-card>
			</v-dialog>
		</div>
		...
		<div class="text-center">
			<v-dialog v-model="policeDialog" persistent width="500">
				<v-card>
					<v-card-title class="headline grey darken-2" primary-title>Police</v-card-title>

					<v-card-text style="text-align:center;">{{policeDialogMessage}}</v-card-text>
					<v-card-text
						v-if="bigPercentageCut > 0"
						style="text-align:center; color: red;"
					>-{{bigPercentageCut}}%</v-card-text>
					<v-card-text
						v-if="smallPercentageCut > 0"
						style="text-align:center; color: red;"
					>-{{smallPercentageCut}}%</v-card-text>
					...
					<v-card-actions>
						<div class="flex-grow-1"></div>
						<v-btn :disabled="policeDisableClose" color="primary" text @click="closePoliceEvent()">Close</v-btn>
					</v-card-actions>
				</v-card>
			</v-dialog>
		</div>
		...
		<div v-if="!policeDialog && !eventDialog" class="text-center">
			<v-dialog v-model="extraInventoryDialog" width="500">
				<v-card>
					<v-card-title class="headline grey darken-2" primary-title>Extra Inventory</v-card-title>

					<v-card-text
						style="text-align:center;"
					>Would you like to buy an extra {{extraInventoryAmount}} inventory slots for £{{Number(extraInventoryPrice).toLocaleString()}}?</v-card-text>
					...
					<v-layout>
						...
						<v-flex xs4 mt-3 pb-5>
							<v-btn
								:disabled="extraInventoryPrice > cash"
								text
								color="success"
								@click="buyExtraInventory()"
							>Yes</v-btn>
						</v-flex>
					</v-layout>
					...
					<v-card-actions>
						<div class="flex-grow-1"></div>
						<v-btn color="primary" text @click="extraInventoryDialog = false">Close</v-btn>
					</v-card-actions>
				</v-card>
			</v-dialog>
		</div>
		...
		<v-layout wrap>
			...
			<v-flex xs12 md4 offset-md4 mb-3>
				<v-card style="text-align:center;">
					<v-layout pt-2>
						<v-flex xs6>
							<h2 class="font-weight-bold mb-3">Day {{days}}/30</h2>
						</v-flex>
						<v-flex xs6>
			...
			<v-flex xs12 md4 offset-md4>
				<v-layout row>
					<product
						@updateCashBuy="updateCashBuy"
						@updateCashSell="updateCashSell"
						:products="products"
...
<script>
	...
	export default {
		...
		data: () => ({
			...
			policeDialogMessage:
				"Police approach you as they have suspicion of your activites and want to bring you in for questioning. Do you want to co operate or run?",
			bigPercentageCut: 0,
			...
			products: [
				{
					...
					outrageousEvent: {
						type: 1,
						message:
							"Cops have made a massive drug bust, cocaine prices have skyrocketed!"
					}
				},
				{
					id: 1,
				...
				},
				{
					...
					outrageousEvent: {
						type: 1,
						message:
							"A strange blue coloured meth has got people hooked. People are paying outrageous amounts"
					}
				},
				{
					id: 7,
		...
		methods: {
			...
			randomiseEvent() {
				...
				if (self.days > 10 && eventChance < 0.12) {
					...
					for (let index = 0; index < 2; index++) {
						let randProdId = Math.floor(
							Math.random() * (self.products.length - 0) + 0
						);

						if (
							!usedEvents.includes(randProdId) &&
							(self.products[randProdId].bottomEvent ||
								self.products[randProdId].outrageousEvent)
						) {
			...
			randomiseEventProducts(randProdId) {
				...
				if (
					(!self.products[randProdId].bottomEvent &&
						self.products[randProdId].outrageousEvent) ||
					(Math.random() < 0.5 && self.products[randProdId].outrageousEvent)
				) {
					return {
						id: randProdId,
			...
			actionPoliceEvent(event) {
				...
				switch (event) {
					case 1:
						let bigPercentage = Math.random() * (0.7 - 0.6) + 0.6;
	            ...
	            bank so finesses you for a ${self.bigPercentageCut}% cut of your cash`;
							...
							// cash - percentage
							self.cash = Math.floor(
								self.cash - self.cash * bigPercentage.toFixed(2)
							);
						} else {
	            ...
	            ${self.smallPercentageCut}% cut for destroying the evidence`;
							...
							// cash - percentage
							self.cash = Math.floor(
								self.cash - self.cash * smallPercentage.toFixed(2)
							);
						}
					...
					case 2:
						...
						if (Math.random() < 0.25) {
							self.policeDialogMessage =
								"You tried to run, but got caught. Police confiscated all your product and half your cash";
